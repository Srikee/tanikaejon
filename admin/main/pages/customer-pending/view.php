<?php
    ...
    if( $search!="" ) {
        $condition .= " AND (
            c.customer_name LIKE '%".$DB->Escape($search)."%'
        )";
    }
    $sql = "
        SELECT
...
<div class="ks-main-header">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            <li class="breadcrumb-item"><a href="./?page=customer-pending">ข้อมูลลูกค้าใหม่</a></li>
        </ol>
    </nav>
</div>
                            ...
                            echo '
                                <tr data-json="'.htmlspecialchars(json_encode($value)).'">
                                    ...
                                    <td class="text-center p-0 pt-1">
                                        <button title="ตรวจสอบข้อมูล" class="btn btn-success btn-sm btn-check-data">
                                            ตรวจสอบข้อมูล
                                        </button>
                                    </td>
                                </tr>
