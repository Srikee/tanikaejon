<?php
    ...
    if( $search!="" ) {
        $condition .= " AND (
            p.staff_name LIKE '%".$DB->Escape($search)."%'
            OR p.staff_sname LIKE '%".$DB->Escape($search)."%'
        )";
    }
    $sql = "
        SELECT
...
<div class="ks-main-content">
    <div class="row mb-3">
        <div class="col">
            <a href="./?page=staff-add" class="btn btn-success me-2" staff="เพิ่มข้อมูล">
                <i class="fa fa-plus me-1"></i>
                เพิ่มข้อมูล
            </a>
            <a href="./?page=staff" class="btn btn-light me-2 border" staff="รีโหลด">
        ...
        <div class="col">
            <div>
                ...
                <input type="text" class="form-control" id="search" value="<?php echo htmlspecialchars($search); ?>"
                    placeholder="ค้นหา">
                <div class="form-text">พิมพ์คำค้นหา.. เจ้าหน้าที่</div>
            </div>
        </div>
    </div>
    <div class="row mb-3">
                ...
                <?php 
                    if( sizeof($obj)==0 ) {
                        ...
                        foreach ($obj as $key => $value) {
                            ...
                            $btn_edit = '<a href="./?page=staff-edit&staff_id='.$value["staff_id"].'" title="แก้ไขข้อมูล" data-container="#table" class="btn btn-warning btn-sm"><i class="fa fa-pencil-alt"></i></a>';
