<?php
    ...
    if( $search!="" ) {
        $condition .= " AND (
            p.provider_name LIKE '%".$DB->Escape($search)."%'
            OR p.provider_sname LIKE '%".$DB->Escape($search)."%'
        )";
    }
    $sql = "
        SELECT
            ...
            o.occupation_name
        FROM provider p
            LEFT JOIN occupation o ON o.occupation_id=p.occupation_id
        WHERE 1=1 ".$condition."
        ORDER BY p.edit_when DESC
    ";
    $show = $SHOW;
...
<div class="ks-main-content">
    <div class="row mb-3">
        <div class="col">
            <a href="./?page=provider-add" class="btn btn-success me-2" provider="เพิ่มข้อมูล">
                <i class="fa fa-plus me-1"></i>
                เพิ่มข้อมูล
            </a>
            <a href="./?page=provider" class="btn btn-light me-2 border" provider="รีโหลด">
        ...
        <div class="col">
            <div>
                ...
                <input type="text" class="form-control" id="search" value="<?php echo htmlspecialchars($search); ?>"
                    placeholder="ค้นหา">
                <div class="form-text">พิมพ์คำค้นหา.. ผู้ให้บริการ</div>
            </div>
        </div>
    </div>
    <div class="row mb-3">
        ...
        <table id="table" class="table table-bordered table-hover">
            <thead>
                <tr class="table-secondary">
                    ...
                    <th style="min-width: 200px;">ชื่อผู้ให้บริการ</th>
                ...
                <?php 
                    if( sizeof($obj)==0 ) {
                        ...
                        foreach ($obj as $key => $value) {
                            ...
                            $btn_edit = '<a href="./?page=provider-edit&provider_id='.$value["provider_id"].'" title="แก้ไขข้อมูล" data-container="#table" class="btn btn-warning btn-sm"><i class="fa fa-pencil-alt"></i></a>';
                            ...
                            echo '
                                <tr data-json="'.htmlspecialchars(json_encode($value)).'">
                                    ...
                                    <td>'.$value["provider_name"].' '.$value["provider_sname"].'</td>
